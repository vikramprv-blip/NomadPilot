    ...
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '32px 0' }}>
        ...
        <div style={{ display: 'inline-flex', background: 'var(--navy-mid)', border: '1px solid var(--navy-border)', borderRadius: 10, padding: 4, gap: 4 }}>
          {(['monthly','annual'] as const).map(b => (
            <button key={b} onClick={() => setBilling(b)} style={{ padding: '8px 24px', borderRadius: 7, border: 'none', cursor: 'pointer', fontFamily: 'DM Sans', fontWeight: 600, fontSize: 13, background: billing === b ? 'var(--gold)' : 'transparent', color: billing === b ? 'var(--navy)' : 'var(--text-dim)', transition: 'all 0.2s' }}>
              {b === 'monthly' ? 'Monthly' : <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>Annual <span style={{ fontSize: 10, background: 'rgba(45,212,160,0.2)', color: 'var(--green)', padding: '1px 6px', borderRadius: 4 }}>Save 20%</span></span>}
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 48 }}>
        {PLANS.map(plan => {
          ...
          return (
            <div key={plan.id} style={{ border: `2px solid ${plan.highlight ? col : 'var(--navy-border)'}`, borderRadius: 14, padding: 24, background: plan.highlight ? `${col}08` : 'var(--navy-mid)', position: 'relative', boxShadow: plan.highlight ? `0 0 28px ${col}20` : 'none' }}>
              {(plan as any).badge && (
                <div style={{ position: 'absolute', top: -13, left: '50%', transform: 'translateX(-50%)', whiteSpace: 'nowrap' }}>
                  <span style={{ background: 'var(--navy)', border: `1px solid ${col}`, color: col, borderRadius: 20, padding: '3px 12px', fontSize: 11, fontWeight: 700, letterSpacing: '0.06em' }}>{(plan as any).badge}</span>
                </div>
              )}
              <h3 style={{ fontSize: 16, fontWeight: 800, letterSpacing: '0.06em', marginBottom: 4, color: (plan as any).color || 'var(--text)' }}>{plan.name}</h3>
              <div style={{ marginBottom: 8 }}>
                <span style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 42, fontWeight: 700, color: (plan as any).color || 'var(--text)' }}>${price}</span>
                <span style={{ fontSize: 13, color: 'var(--text-dim)' }}>/mo</span>
                {billing === 'annual' && price > 0 && <div style={{ fontSize: 11, color: 'var(--green)' }}>Billed ${plan.price.annual}/year</div>}
              </div>
              <p style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 20, fontStyle: 'italic' }}>{plan.tagline}</p>
              <p style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 16, fontStyle: 'italic' }}>See full feature comparison below</p>

              {isCurrent ? (
                <button style={{ width: '100%', padding: '10px', borderRadius: 8, border: '1px solid var(--navy-border)', background: 'var(--navy-light)', color: 'var(--text-muted)', cursor: 'default', fontFamily: 'DM Sans', fontWeight: 600, fontSize: 13 }}>Current Plan</button>
              ) : price === 0 ? (
                <button className="btn btn-navy" style={{ width: '100%', justifyContent: 'center' }}>Get Started Free</button>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <button onClick={() => handleSubscribe(plan.id, 'stripe')} disabled={loading === `${plan.id}-stripe`} style={{ width: '100%', padding: '10px', borderRadius: 8, border: 'none', background: (plan as any).color || 'var(--gold)', color: 'white', cursor: 'pointer', fontFamily: 'DM Sans', fontWeight: 700, fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                    {loading === `${plan.id}-stripe` ? <span className="spin">◌</span> : '💳'} Choose Plan
                  </button>
                  <button onClick={() => handleSubscribe(plan.id, 'paypal')} disabled={loading === `${plan.id}-paypal`} className="btn btn-navy" style={{ width: '100%', justifyContent: 'center', fontSize: 12 }}>
                    🅿 PayPal
                  </button>
                </div>
              )}
      ...
      <div>
        <h3 style={{ fontSize: 26, fontWeight: 700, textAlign: 'center', marginBottom: 8 }}>Compare Plan Features</h3>
        <p style={{ textAlign: 'center', color: 'var(--text-dim)', fontSize: 14, marginBottom: 28 }}>
          Find the perfect plan for your travel needs. From basic trip planning to elite concierge services, we have you covered.
        </p>

        <div style={{ background: 'var(--navy-mid)', borderRadius: 14, overflow: 'hidden', border: '1px solid var(--navy-border)' }}>
          {/* Table header */}
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', background: 'var(--navy)', padding: '14px 20px', borderBottom: '1px solid var(--navy-border)' }}>
            <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--gold)' }}>Features</div>
            {['Free','Premium','Pro','Elite'].map((p, i) => (
              <div key={p} style={{ textAlign: 'center', fontWeight: 700, fontSize: 13, color: [undefined,'#3b82f6','#8b5cf6','#e8a020'][i] || 'var(--text-dim)' }}>{p}</div>
            ))}
          </div>

          {FEATURES.map(group => (
            <div key={group.category}>
              <div style={{ padding: '10px 20px', background: 'rgba(232,160,32,0.04)', borderBottom: '1px solid var(--navy-border)', borderTop: '1px solid var(--navy-border)' }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-dim)', letterSpacing: '0.08em' }}>{group.category}</span>
              </div>
              {group.items.map((item, i) => (
                <div key={item.name} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', padding: '12px 20px', borderBottom: '1px solid var(--navy-border)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)' }}>
                  ...
                  {(['free','premium','pro','elite'] as const).map(p => (
                    <div key={p} style={{ textAlign: 'center', fontSize: 16 }}>
                      {item[p] ? <span style={{ color: 'var(--green)' }}>✓</span> : <span style={{ color: 'var(--text-muted)' }}>—</span>}
                    </div>
                  ))}
                </div>
              ))}
