    ...
    <div className="fade-up" style={{maxWidth:880,margin:'0 auto'}}>
      <div style={{textAlign:'center',marginBottom:36}}>
        ...
        <p style={{color:'var(--text-dim)',fontSize:16,maxWidth:440,margin:'0 auto'}}>
          Search manually or let our AI plan your entire journey.
        </p>
      </div>
        ...
        {tab==='ai' ? (
          <div>
            <textarea className="input-field" value={aiText} onChange={e=>setAiText(e.target.value)}
              ...
              placeholder="e.g. Fly me from Dubai to Tokyo next month, business class, 2 people, budget $6000..."
              rows={4} style={{resize:'none',marginBottom:12,lineHeight:1.6}} />
            <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:16}}>
              {['NYC to London, business, next Friday','Dubai to Bali, eco hotels, 2 weeks','Multi-city: Paris then Rome then Barcelona'].map(ex=>(
          ...
          <div>
            ...
            {tripType==='multicity' ? (
                ...
                {legs.map((leg,i)=>(
                  <div key={i} style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr auto',gap:8,alignItems:'center'}}>
                    <div style={{position:'relative'}}>
                      ...
                      <input className="input-field" style={{paddingLeft:30}} placeholder="From" value={leg.from} onChange={e=>updateLeg(i,'from',e.target.value)} />
                    </div>
                    <div style={{position:'relative'}}>
                      <span style={{position:'absolute',left:10,top:'50%',transform:'translateY(-50%)',fontSize:13}}>📍</span>
                      <input className="input-field" style={{paddingLeft:30}} placeholder="To" value={leg.to} onChange={e=>updateLeg(i,'to',e.target.value)} />
                    </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:`1fr 1fr 1fr${tripType==='return'?' 1fr':''}`,gap:12,marginBottom:16}}>
                <div style={{position:'relative'}}>
                  <span style={{position:'absolute',left:10,top:'50%',transform:'translateY(-50%)',fontSize:13}}>📍</span>
                  <input className="input-field" style={{paddingLeft:30}} placeholder="From — City or IATA" value={mainLeg.from} onChange={e=>setMainLeg(p=>({...p,from:e.target.value}))} />
                </div>
                <div style={{position:'relative'}}>
                  <span style={{position:'absolute',left:10,top:'50%',transform:'translateY(-50%)',fontSize:13}}>📍</span>
                  <input className="input-field" style={{paddingLeft:30}} placeholder="To — City or IATA" value={mainLeg.to} onChange={e=>setMainLeg(p=>({...p,to:e.target.value}))} />
                </div>
                <div style={{position:'relative'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12,marginBottom:20}}>
              ...
              <div>
                <label style={lbl}>Nationality (visa check)</label>
            ...
            <button className="btn btn-gold btn-lg" onClick={()=>onSubmit({tripType,services,nationality,legs:tripType==='multicity'?legs:[mainLeg],travelers,cabinClass})} disabled={!canSearch||loading} style={{width:'100%',justifyContent:'center'}}>
