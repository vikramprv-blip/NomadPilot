...
export default function BetaPage() {
  const [form, setForm] = useState({
    email: '', name: '', country: '', travel_type: '', how_heard: '', use_case: '',
  });
    ...
    <div style={{ minHeight: '100vh', background: '#0a1628', color: '#fff', fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <div style={{ textAlign: 'center', padding: '80px 24px 48px' }}>
        ...
        <h1 style={{ fontSize: 52, fontWeight: 700, fontFamily: 'Cormorant Garamond, serif', marginBottom: 16, lineHeight: 1.15 }}>
          Travel smarter.<br />
          <span style={{ color: '#e8a020' }}>AI-powered.</span>
        </h1>
        ...
        <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.35)', marginBottom: 48 }}>
          Limited spots. Join the waitlist and get early access.
        </p>
      ...
      <div style={{ maxWidth: 520, margin: '0 auto', padding: '0 24px 80px' }}>
        {status === 'success' ? (
          <div style={{ textAlign: 'center', padding: '48px 32px', background: 'rgba(45,212,160,0.08)', borderRadius: 16, border: '1px solid rgba(45,212,160,0.2)' }}>
            ...
            {result?.invite_code && (
              <div style={{ background: 'rgba(232,160,32,0.1)', borderRadius: 10, padding: '14px 20px', border: '1px solid rgba(232,160,32,0.2)' }}>
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', marginBottom: 4 }}>Your invite code</div>
                <div style={{ fontSize: 22, fontWeight: 700, letterSpacing: '0.1em', color: '#e8a020', fontFamily: 'monospace' }}>
                  {result.invite_code}
                </div>
              </div>
            )}
            <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', marginTop: 16 }}>
              Share NomadPilot with friends — help us grow the beta 🚀
            </p>
            <div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginTop: 12, flexWrap: 'wrap' }}>
              {[
                { label: '𝕏 Twitter', url: `https://twitter.com/intent/tweet?text=Just+joined+the+%40NomadPilot+beta+%E2%80%94+AI-powered+flight+search+in+your+own+currency.+Join+the+waitlist%3A+${encodeURIComponent(typeof window !== 'undefined' ? window.location.href : '')}` },
                { label: 'LinkedIn', url: `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(typeof window !== 'undefined' ? window.location.href : '')}` },
              ].map(s => (
                <a key={s.label} href={s.url} target="_blank" rel="noopener noreferrer"
                  style={{ padding: '8px 18px', borderRadius: 7, background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', color: '#fff', fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>
                  {s.label}
                </a>
              ))}
            </div>
            ...
            <div style={{ display: 'grid', gap: 16 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  ...
                  <input style={inputStyle} type="text" placeholder="e.g. Denmark" value={form.country} onChange={e => set('country', e.target.value)} />
                </div>
                <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g. planning monthly business trips to India, comparing flight prices..."
                  value={form.use_case}
              ...
              <button
                ...
                style={{ width: '100%', padding: '14px', borderRadius: 10, background: form.email ? '#e8a020' : 'rgba(232,160,32,0.3)', border: 'none', color: form.email ? '#0a1628' : 'rgba(10,22,40,0.5)', fontSize: 15, fontWeight: 700, cursor: form.email ? 'pointer' : 'not-allowed', fontFamily: 'DM Sans, sans-serif', transition: 'all 0.2s' }}
              >
                {status === 'loading' ? '⏳ Joining...' : '🚀 Join Beta Waitlist'}
              </button>

              {status === 'error' && (
                <p style={{ color: '#ff6b6b', fontSize: 13, textAlign: 'center' }}>Something went wrong. Please try again.</p>
              )}

              <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)', textAlign: 'center', marginTop: 4 }}>
                No spam. We'll only email you with your invite.
              </p>
            </div>
          </div>
        )}
